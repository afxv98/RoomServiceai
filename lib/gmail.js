...
/** Read admin refresh token from request cookie, falling back to env var. */
...
/** Read sales rep refresh token from request cookie. */
export function getSalesRefreshToken(request) {
  const cookieHeader = request.headers.get('cookie') || '';
  const match = cookieHeader.match(/sales_gmail_refresh_token=([^;]+)/);
