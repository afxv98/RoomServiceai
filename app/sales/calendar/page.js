...
import {
  ChevronLeft, ChevronRight, Plus, X, Calendar, Clock, Link2, Trash2,
} from 'lucide-react';
        ...
        <div className="p-6 space-y-4">
          ...
          {event?.lead && (
            <div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
              ...
              <span>Linked to: <strong>{event.lead.hotelName}</strong></span>
            </div>
          )}
        </div>
        ...
        <div className="px-6 pb-5 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="px-5 py-2 text-sm bg-copper text-white rounded-lg hover:bg-copper/90 disabled:opacity-50 font-medium transition-colors"
            >
...
export default function SalesCalendarPage() {
  ...
  const eventsOnDay = (date) => {
    ...
    return events.filter(e => new Date(e.startTime).toDateString() === d);
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-charcoal">My Calendar</h1>
          <p className="text-gray-400 text-sm">Your events, demos & reminders</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() => setCurrent(view === 'month' ? new Date(today.getFullYear(), today.getMonth(), 1) : new Date(today))}
            className="px-3 py-1.5 text-sm rounded-lg hover:bg-gray-100 transition-colors text-gray-700 font-medium"
          >
            Today
          </button>
                ...
                {monthGrid.map((date, i) => {
                  ...
                  return (
                    <div
                      ...
                      className="h-28 border-r border-b border-gray-100 p-1.5 cursor-pointer hover:bg-gray-50 transition-colors group"
                      onClick={() => setModal({ event: null, defaultDate: date })}
                    >
                      <div className={`text-xs font-semibold mb-1 w-6 h-6 flex items-center justify-center rounded-full ${
                        isToday ? 'bg-copper text-white' : 'text-gray-600 group-hover:text-charcoal'
                      }`}>
                        {date.getDate()}
                      </div>
              ...
              <div className="grid grid-cols-7 border-b border-gray-100">
                {weekDays.map(date => {
                  ...
                  return (
                    <div
                      key={date.toISOString()}
                      className="py-3 text-center border-r border-gray-100 last:border-r-0 cursor-pointer hover:bg-gray-50"
                      onClick={() => setModal({ event: null, defaultDate: date })}
                    >
              ...
              <div className="grid grid-cols-7 min-h-64">
                {weekDays.map(date => {
                  ...
                  return (
                    <div
                      key={date.toISOString()}
                      className="border-r border-gray-100 last:border-r-0 p-2 space-y-1 min-h-48 cursor-pointer hover:bg-gray-50/50"
                      onClick={() => setModal({ event: null, defaultDate: date })}
                    >
                      {dayEvents.length === 0 && <div className="text-xs text-gray-300 text-center pt-4">—</div>}
                      {dayEvents.map(ev => {
                        const ti = typeInfo(ev.type);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h2 className="text-sm font-semibold text-charcoal mb-3 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-copper" /> Upcoming
        </h2>
        {events
          ...
          .map(ev => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ti.light} ${ti.text}`}>{ti.label}</span>
              </div>
            );
          })}
