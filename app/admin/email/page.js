...
// ─── AttachmentChip (received) ────────────────────────────────────────────────

function AttachmentChip({ att }) {
  const Icon = attachmentIcon(att.mimeType);
  const href =
    `/api/gmail/attachment/${att.messageId}/${att.attachmentId}` +
    `?filename=${encodeURIComponent(att.filename)}&type=${encodeURIComponent(att.mimeType)}`;
  return (
    <a
      href={href}
      download={att.filename}
      className="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 border border-gray-200
                 text-gray-700 rounded-lg px-3 py-2 text-xs font-medium transition-colors group"
    >
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full py-24 text-center px-6">
      ...
      <p className="text-gray-500 max-w-sm mb-6 text-sm leading-relaxed">
        Link your Gmail account to manage all customer emails directly from the admin panel.
      </p>
      {msg && (
      ...
      <a href="/api/gmail/auth" className="inline-flex items-center gap-2 bg-copper text-white px-6 py-3 rounded-lg font-semibold hover:bg-copper-dark transition-all duration-200 text-sm">
    ...
    <div className="col-span-1 border-r border-gray-200 flex flex-col overflow-hidden">
      <div className="p-4 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={onCompose}
            className="inline-flex items-center gap-1.5 bg-copper text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-copper-dark transition-colors"
          ...
          <button
            onClick={onRefresh}
            disabled={loading}
            className="p-1.5 rounded hover:bg-gray-100 text-gray-500 hover:text-charcoal transition-colors disabled:opacity-40"
            title="Refresh"
          >
...
function MessageBubble({ msg }) {
  const isOutgoing = msg.from?.toLowerCase().includes('me') || false;
  return (
    <div className={`mb-4 ${isOutgoing ? 'ml-8' : ''}`}>
      <div className={`rounded-lg p-4 ${isOutgoing ? 'bg-copper/10 border border-copper/20' : 'bg-gray-50 border border-gray-200'}`}>
...
function ThreadDetail({ thread, messages, loadingMessages, onSend, sending }) {
  ...
  const handleFileChange = async (e) => {
    ...
    const read = await Promise.all(selected.map(readFileAsBase64));
    setAttachedFiles((prev) => {
      const existing = new Set(prev.map((f) => f.name));
      return [...prev, ...read.filter((f) => !existing.has(f.name))];
    });
    e.target.value = '';
  };
  ...
  const handleSend = async () => {
    ...
    const ok = await onSend({
      ...
      references: latestMsg?.references || latestMsg?.messageId || '',
      attachments: attachedFiles.map(({ name, type, data }) => ({ name, type, data })),
    });
    ...
    <div className="col-span-2 flex flex-col overflow-hidden">
      ...
      <div className="p-5 border-t border-gray-200 flex-shrink-0">
        <textarea
          ...
          className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm text-charcoal placeholder-gray-400 focus:outline-none focus:border-copper focus:ring-1 focus:ring-copper/30 resize-none transition-colors"
        />
        {attachedFiles.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-2">
            {attachedFiles.map((f) => (
              <FileChip key={f.name} file={f} onRemove={(name) => setAttachedFiles((prev) => prev.filter((x) => x.name !== name))} />
            ))}
          </div>
        )}
        ...
        <div className="flex items-center gap-2 mt-3">
          <button
            ...
            className="inline-flex items-center gap-2 bg-copper text-white px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-copper-dark transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
          >
...
function ComposeModal({ isOpen, onClose, onSend, sending }) {
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <h2 className="font-bold text-charcoal flex items-center gap-2">
            <Pencil className="w-4 h-4 text-copper" />
            New Message
          </h2>
          <button onClick={handleClose} className="p-1.5 rounded hover:bg-gray-100 text-gray-400 hover:text-charcoal transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-5 space-y-3">
          <div>
            ...
            <input type="email" value={to} onChange={(e) => setTo(e.target.value)} placeholder="recipient@example.com"
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-charcoal placeholder-gray-400 focus:outline-none focus:border-copper focus:ring-1 focus:ring-copper/30 transition-colors" />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={body} onChange={(e) => { setBody(e.target.value); if (sendError) setSendError(''); }}
              rows={7} placeholder="Write your message…"
              className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm text-charcoal placeholder-gray-400 focus:outline-none focus:border-copper focus:ring-1 focus:ring-copper/30 resize-none transition-colors" />
          </div>
          {attachedFiles.length > 0 && (
        ...
        <div className="flex items-center gap-2 px-5 py-4 border-t border-gray-200">
          ...
          <input ref={fileInputRef} type="file" multiple className="hidden" onChange={handleFileChange} />
          <button onClick={handleClose} className="ml-auto text-sm text-gray-400 hover:text-charcoal transition-colors">
            Discard
          </button>
        </div>
      </div>
  ...
  return (
    <div className="col-span-2 flex flex-col items-center justify-center h-full text-center px-8">
      ...
      <button onClick={onCompose}
        className="inline-flex items-center gap-2 border border-copper text-copper px-4 py-2 rounded-lg text-sm font-semibold hover:bg-copper hover:text-white transition-colors">
...
export default function EmailHubPage() {
  ...
  const selectThread = async (thread) => {
    ...
    try {
      const res = await fetch(`/api/gmail/thread/${thread.id}`);
      const data = await res.json();
  ...
  const handleSend = async (payload) => {
    setSending(true);
    try {
      const res = await fetch('/api/gmail/send', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!data.success) throw new Error();
      const refreshRes = await fetch(`/api/gmail/thread/${payload.threadId}`);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="mb-6 flex items-center justify-between flex-shrink-0">
        <div>
          <h1 className="text-3xl font-bold text-charcoal font-playfair mb-1">Email Hub</h1>
          <p className="text-gray-500 text-sm">
            {status === 'connected' && connectedEmail ? `Connected as ${connectedEmail}` : 'Manage customer communications'}
          </p>
        </div>
        {status === 'connected' && (
          <div className="flex items-center gap-3">
            <button onClick={() => setComposing(true)}
              className="inline-flex items-center gap-2 bg-copper text-white px-4 py-2.5 rounded-lg text-sm font-semibold hover:bg-copper-dark transition-colors">
            ...
            <a href="/api/gmail/auth" className="text-xs text-gray-500 hover:text-charcoal underline transition-colors">Reconnect</a>
          </div>
        )}
      </div>
      ...
      <ComposeModal isOpen={composing} onClose={() => setComposing(false)} onSend={handleComposeSend} sending={composeSending} />
    </div>
  );
}
